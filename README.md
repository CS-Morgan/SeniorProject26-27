# CS460-Fall26-ClassRepo
## Miscellaneous
.NET 10.0 is the current "main" release.  Let's use that one.
```
dotnet --list-sdks
dotnet help
dotnet new -h
...
Creates (or uses) a folder named Sample, with subfolders Sample_Code and Sample_Tests.  Places a solution named "Sample"  in the outer folder, a MVC project named Sample in the Sample/Sample folder and an NUnit project in the Sample/Sample_Tests folder.  Adds a reference from the testing project to the main project so it can be used to test code in the main project.
```
...
```
#cd into the project you want to add to, i.e. Sample/Sample_Code (so you're right next to the .csproj file)
dotnet list package
...
{
    // adds the following logging providers: Console, Debug, EventSource, EventLog (https://learn.microsoft.com/en-us/aspnet/core/fundamentals/logging/?view=aspnetcore-10.0)
